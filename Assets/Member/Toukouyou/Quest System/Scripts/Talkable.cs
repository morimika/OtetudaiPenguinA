...
public class Talkable : MonoBehaviour
{
    ...
    [SerializeField] private float _talkableSizeX;
    [SerializeField] private float _talkableSizeY;
    private void Start()
    {
    ...
    private void Update()
    {
        ...
        if (_isTalkable && _dialogueBox.activeInHierarchy == true && _dialogueBox_talking.activeInHierarchy == false &&DialogManager.instance._waitTimeController == true && DialogManager.instance._testClick == true)
        {
            if (Input.GetMouseButtonDown(0))
            {
                ...
                if (x < NPC_Pos.position.x + _talkableSizeX && x > NPC_Pos.position.x - _talkableSizeX &&
                   y < NPC_Pos.position.y + _talkableSizeY && y > NPC_Pos.position.y - _talkableSizeY)
                {
                    Debug.Log(NPC_Pos);
